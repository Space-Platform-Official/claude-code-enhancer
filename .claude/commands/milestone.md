...
# 🔀 COMMAND SEPARATION MIGRATION NOTICE

**⚠️ IMPORTANT: The milestone command has been separated into specialized commands for better usability!**

## New Command Structure

The milestone functionality has been split into focused, semantic commands:

### 📊 **Status Monitoring** → `/milestone/status`
- **Purpose**: Read-only status display and progress monitoring
- **Use for**: Checking milestone progress, viewing task status, dependency tracking
- **Example**: `/milestone/status milestone-003`

### 🎨 **Visualization** → `/milestone/visualize`  
- **Purpose**: Generate charts, graphs, and visual representations
- **Formats**: Text, HTML, SVG, JSON output
- **Use for**: Progress dashboards, timeline charts, dependency graphs
- **Example**: `/milestone/visualize milestone-003 --format=html`

### ✏️ **State Updates** → `/milestone/update`
- **Purpose**: Modify milestone data and progress
- **Use for**: Updating progress, adding tasks, changing status, managing dependencies
- **Example**: `/milestone/update milestone-003 --progress=75`

### 📊 **Interactive Dashboard** → `/milestone/dashboard`
- **Purpose**: Real-time monitoring with live updates
- **Use for**: Team collaboration, live progress tracking, notifications
- **Example**: `/milestone/dashboard --live --team`
...
```bash
# Status checking
/milestone/update status           → /milestone/status
/milestone/update check            → /milestone/status
/milestone/update monitor          → /milestone/status

# Visualization  
/milestone/update dashboard        → /milestone/visualize --format=html
/milestone/update chart            → /milestone/visualize --format=svg
/milestone/update report           → /milestone/visualize --format=text

# State modifications
/milestone/update progress 75%     → /milestone/update --progress=75
/milestone/update complete task    → /milestone/update --complete-task=task-id
/milestone/update add task         → /milestone/update --add-task="description"

# Interactive monitoring
/milestone/update live             → /milestone/dashboard --live
/milestone/update team-view        → /milestone/dashboard --team
```

## Why the Change?

✅ **Clear separation of concerns**: Read vs Write operations are distinct
✅ **Better performance**: Specialized commands load faster
✅ **Improved usability**: Each command has focused, relevant options
✅ **Easier maintenance**: Smaller, focused codebases
✅ **Enhanced features**: Each command can have specialized capabilities

## Backward Compatibility

The original `/milestone` command continues to work for **milestone planning and creation**. The separated commands handle **monitoring and updates** of existing milestones.

**Continue using `/milestone` for:**
- Creating new milestone plans
- Initial project decomposition
- Setting up milestone tracking systems

**Use the new commands for:**
- Day-to-day milestone management

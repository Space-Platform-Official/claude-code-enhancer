...
handle_environment_error() {
    ...
    echo "1. Try running with explicit bash: /bin/bash ${SCRIPT_NAME}" >&2
...
merge_file() {
    ...
    if [[ -f "${target_file}" ]]; then
        ...
        if ! mkdir -p "${target_dir_path}"; then
            print_error "Failed to create directory: ${target_dir_path}"
            return 1
        fi
...
merge_claude_files() {
    ...
    if ! source_dir="$(find_templates_dir)"; then
        ...
        print_error "  1. \$CLAUDE_TEMPLATE_SOURCE: ${CLAUDE_TEMPLATE_SOURCE:-not set}"
        print_error "  2. \$CLAUDE_TEMPLATES_DIR: ${CLAUDE_TEMPLATES_DIR:-not set}"
        print_error "  3. ~/.local/share/claude-flow/templates"
        print_error "  4. /usr/local/share/claude-flow/templates"
        print_error "  5. ${SCRIPT_DIR}"
        return 1
    fi
    ...
    if [[ -d "${source_dir}/.claude" ]] && [[ -n "$(ls -A "${source_dir}/.claude" 2>/dev/null || true)" ]]; then
...
main() {
    ...
    if [[ ${env_warnings} -gt 0 ]]; then
        ...
        print_warning "If you encounter issues, try running with: /bin/bash ${SCRIPT_NAME}"
        echo ""
    fi
